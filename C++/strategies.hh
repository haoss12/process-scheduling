...
{
    _cpu = std::make_shared<cpu>(n);    //tworzymy nowy procesor o zadanej liczbie rdzeni 
}
...
void scheduler::schedule()
{
    ...
    {
        if(_cpu->thread[i].process_id != -1) //zmniejszamy czas wykonywania sie zadania co takt procesora
        {  
...
bool scheduler::is_cpu_free() const
{
    for(size_t i = 0; i < _cpu->cores_count; ++i)
    {
        if(_cpu->thread[i].process_id != INACTIVE)    //jesli jest jakiekolwiek zadanie - falsz
        {
            return false;
        }
    }
...
void scheduler::run()
{
    ...
    while(std::cin >> timestamp)    //wczytywanie czasu do momentu kiedy nie skoncza sie dane
    {
        std::getline(std::cin, line);   //wczytanie reszty linii z pliku wejsciowego
        std::stringstream args(line);   //konwersja linii tak aby wygodnie odczytac z niej dane 

        while(args >> temp)
        {
            p_queue.push_back(temp);    //dodanie nowego procesu do kolejki 
        }
        
        schedule(); //uszeregowanie zadan 
        std::cout << _cpu->print() << std::endl;
    }

    while(!is_cpu_free())   //po skonczeniu sie danych szeregujemy zadania do momentu kiedy procesor jest pusty
    {
        schedule();
        std::cout << _cpu->print() << std::endl;        
    }
...
void scheduler::add_to_cpu()
{
    for(size_t i = 0; i < _cpu->cores_count; ++i)
    {
        if(_cpu->thread[i].process_id == INACTIVE)      //jeśli rdzeń jest wolny dodajemy zadanie z kolejki...
        {
            if(!p_queue.empty())                        //...chyba ze rowniez ona jest pusta
            {
...
void scheduler::fcfs()  //first come first serve - dodajemy zadania po kolei
{
    add_to_cpu();
}

void scheduler::sjf()   //shortest jobtime first - sortujemy procesy pod wzgledem czasu wykonania
{
...
void scheduler::srtf()  //najkrotszy pozostaly czas - z wywlaszczaniem
{
...
void scheduler::rr()    //co kwant czasu zrzucamy wszystko z procesora i dodajemy na nowo
{
...
void scheduler::pnd()   //priorytetowe bez wywlaszczania - sortujemy zadania po priorytecie 
{
